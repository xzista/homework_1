# Виджет банковских операций клиента

## Описание:

Разработка виджета банковских операций клиента (*будет дополнено по мере разработки*).

## Установка:

1. Клонируйте репозиторий:
```
git clone https://github.com/xzista/homework_1.git
```
2. Установите зависимости:
```
poetry install
```
## Использование:

(*В разработке*)

## Тестирование:

1. Для выполнения тестирования пропишите команду:
```
pytest
```
2. Для получения информации о тестировании пропишите команду:
```
pytest --cov
```

## Модуль Generators

Модуль `generators` предоставляет функции для работы с массивами транзакций. Он включает в себя следующие функции:

- `filter_by_currency(transactions, currency)`: фильтрует транзакции по заданной валюте и возвращает итератор.
- `transaction_descriptions(transactions)`: генератор, возвращающий описания транзакций.
- `card_number_generator(start, stop)`: генератор, который выводит номера банковских карт в формате ХХХХ ХХХХ ХХХХ ХХХХ.

### Примеры использования:

```python
# Пример использования filter_by_currency
usd_transactions = filter_by_currency(transactions, 'USD')
for transaction in usd_transactions:
    print(transaction)

# Пример использования transaction_descriptions
for description in transaction_descriptions(transactions):
    print(description)

# Пример использования card_number_generator
for card in card_number_generator(4000123456789010, 4000123456789015):
    print(card)
```
## Модуль Decorators

Модуль `Decorators` предоставляет декораторы для изменения работы функций. Он включает в себя следующие декораторы:

- `@log()` Декоратор логирования работы функций, с возможностью записи логов в файл.

## Модуль Utils

Модуль `utils` предоставляет функции для работы с JSON - файлами. Он включает в себя следующие функции:

- `convert_json_to_list_of_dict(path_json)`: передавая путь до JSON-файла конвертирует его данные в объекты.

## Модуль External_api

Модуль `external_api` предоставляет функции для работы с api - запросами. Он включает в себя следующие функции:

- `transaction_amount(transaction)`: принимает на вход транзакцию и возвращает сумму транзакции в рублях.

## Модуль Utils

Модуль `csv_and_xlsx_converters` предоставляет функции для работы с CSV и xlsx - файлами.
Он включает в себя следующие функции:

- `convert_csv_to_list_of_dict(path_csv)`: передавая путь до CSV-файла конвертирует его данные в объекты.
- 
- `convert_xlsx_to_list_of_dict(path_xlsx)`: передавая путь до xlsx-файла конвертирует его данные в объекты.

## Модуль main

Модуль `main` предоставляет функции для запуска программы и связи с пользователем.
Он включает в себя следующие функции:

- `main()`: отвечает за основную логику проекта с пользователем
    и связывает функциональности между собой.

## Документация:

Для получения дополнительной информации обратитесь к [документации](README.md).

## Лицензия:

Этот проект лицензирован по лицензии MIT.